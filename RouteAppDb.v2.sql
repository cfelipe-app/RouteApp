/* =========================================================
   RouteApp • Script FINAL (Esquema + Índices + Vistas
   + Triggers + SP + Seed + hooks con Identity)
   SQL Server (T-SQL)
   ========================================================= */
...
-- Route (con DriverUserId para rol Driver)
...
(
    ...
    DriverUserId    NVARCHAR(450)     NULL,      -- FK opcional a AspNetUsers
...
CREATE INDEX IX_Route_ServiceDate_Driver      ON dbo.[Route](ServiceDate, DriverUserId);
GO

-- Si existen tablas de Identity, crea FK Route.DriverUserId -> AspNetUsers(Id)
IF OBJECT_ID('dbo.AspNetUsers','U') IS NOT NULL
BEGIN
    IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name='FK_Route_AspNetUsers_Driver')
    BEGIN
        ALTER TABLE dbo.[Route] WITH CHECK
            ADD CONSTRAINT FK_Route_AspNetUsers_Driver FOREIGN KEY (DriverUserId)
...
------------------------------------------------------------
-- INTEGRACIÓN OPCIONAL CON IDENTITY (multi-tenant proveedor)
-- Si AspNetUsers existe, añade ProviderId y FK a Provider
------------------------------------------------------------
IF OBJECT_ID('dbo.AspNetUsers','U') IS NOT NULL
BEGIN
    -- Añadir columna si no existe
    IF COL_LENGTH('dbo.AspNetUsers','ProviderId') IS NULL
        ALTER TABLE dbo.[AspNetUsers] ADD [ProviderId] INT NULL;

    -- FK a Provider (si no existe)
    IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name='FK_AspNetUsers_Provider')
        ALTER TABLE dbo.[AspNetUsers] WITH CHECK
            ADD CONSTRAINT [FK_AspNetUsers_Provider]
            FOREIGN KEY ([ProviderId]) REFERENCES dbo.[Provider]([Id]) ON DELETE SET NULL;

    -- Índice sobre ProviderId (usar SQL dinámico para evitar error en IF)
    IF NOT EXISTS (
        SELECT 1
        FROM sys.indexes
        WHERE name = 'IX_AspNetUsers_Provider'
          AND object_id = OBJECT_ID('dbo.AspNetUsers')
    )
        EXEC('CREATE INDEX [IX_AspNetUsers_Provider] ON dbo.[AspNetUsers]([ProviderId]);');
END
GO
...
-- Numeración automática V1/V2/... (opcional)
...
INSERT INTO dbo.[Route] (ServiceDate, VehicleId, ProviderId, [Code], [Status], DistanceKm, DurationMin, ColorHex, StartTime, EndTime, DriverUserId)
VALUES
('2025-09-07', 1, 1, 'V1', 'Planned', 50.5, 120, '#FF5733', '08:00', '10:20', NULL),
('2025-09-07', 3, 2, 'V2', 'Planned', 15.0,  60, '#33C1FF', '09:00', '10:00', NULL);
...
VALUES
(NULL, 0, '2025-09-09', N'Lima Metropolitana', 3000, 25, 20, '08:00', '18:00', 'Open', 'admin'),
(2,    0, '2025-09-09', N'Trujillo Centro',      500,  5,  5, '09:00', '15:00', 'Open', 'planner'),
(1,    1, '2025-09-10', N'Lima Metropolitana',  2000, 15, 10, '08:00', '18:00', 'Open', 'planner');
